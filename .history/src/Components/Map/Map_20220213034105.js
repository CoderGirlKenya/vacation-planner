    ...
    <div className={classes.mapContainer}>
      ...
      >
        ...
        {weatherData.main.length && weatherData.main.map((data, i) => (
          <div key={i} lat={data.coord.lat} lng={data.coord.ln}>
             
             <img src={`http://openweathermap.org/img/w/${data.weather[].main}.png`} alt={`http://openweathermap.org/img/w/${data.weather[0].icon}.png`} height="70px" />
          </div>
        ))}
          </div>
        ))}
