    ...
    const [touchFoodActivity, setTouchedFoodActivity] = useState (false);
              ...
              if (validDestination && validOutdoorsActivity && validToursActivity && validFoodActivity){
                 ...
                 const submitHandler = (event) => {
                            try {
                        event.preventDefault();
            let list = document.getElementById("allVacations").value; //get value of variable
            //  let body = document.getElementById("variable").value; //get value of variable
          let response = await fetch("link to json api",{
                        method: "POST",
                        headers: {"Content-Type": "application/json"},
                  body: JSON.stringify({ list : allVacations, body: body })
                                  }
                                );
                    ...
                    // Document.getElementById("vacations").addEventListener("submit", addNewItem);
